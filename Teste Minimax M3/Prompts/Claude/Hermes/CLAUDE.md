...
```text
~/Documents/shopee/hermes/site_hermes/
```

- não criar pasta paralela como `blog_hermes/`
- não criar arquivos do site fora de `site_hermes/`
- preservar arquivos já existentes em `site_hermes/`, principalmente `hero.png` e `data/`
...
site_hermes/data/produtos_site_por_categoria.json
site_hermes/data/produtos_site_por_categoria.csv
```

Para links, priorizar:

```text
site_hermes/data/links_shopee_manual.csv
data/final/links_shopee_manual.csv
```

### Estrutura esperada
Criar dentro de `site_hermes/`:
...
- usar `imagem/prompt_hero_blog_hermes.md` como referência
- se `site_hermes/hero.png` existir, usar esse arquivo
...
- organizar assets em `site_hermes/assets/`
...
- erros encontrados
- decisões tomadas
- confirmação de que nada foi criado fora de `site_hermes/`
